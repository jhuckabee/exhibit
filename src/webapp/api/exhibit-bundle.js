...
Exhibit.AlphaRangeFacet._settingSpecs={"facetLabel":{type:"text"},"scroll":{type:"boolean",defaultValue:true},"height":{type:"text"},"interval":{type:"number",defaultValue:7},"collapsible":{type:"boolean",defaultValue:false},"collapsed":{type:"boolean",defaultValue:false}};
...
this._rangeIndex._pairs.sort(function(G,F){return G.value.localeCompare(F.value);
});
}};
...
/* remote-list-facet.js */
Exhibit.RemoteListFacet=function(B,A){this._div=B;
this._uiContext=A;
this._colorCoder=null;
this._valueSet=new Exhibit.Set();
this._settings={};
...
Exhibit.RemoteListFacet._settingSpecs={"remoteFacetId":{type:"number"},"facetLabel":{type:"text"},"fixedOrder":{type:"text"},"sortMode":{type:"text",defaultValue:"value"},"sortDirection":{type:"text",defaultValue:"forward"},"scroll":{type:"boolean",defaultValue:true},"height":{type:"text"},"collapsible":{type:"boolean",defaultValue:false},"collapsed":{type:"boolean",defaultValue:false},"selectMultiple":{type:"boolean",defaultValue:false},"sourceUrl":{type:"text"},"sourceUrlType":{type:"text",defaultValue:"application/json"},"sourceExpression":{type:"text"},"queryParamName":{type:"text",defaultValue:"args"},"queryParamSeparator":{type:"text",defaultValue:","}};
Exhibit.RemoteListFacet.create=function(E,C,A){var A=Exhibit.UIContext.create(E,A);
var B=E.remoteFacetId;
if(B!=null&&B.length>0&&Exhibit.RemoteListFacet._instances.length>0&&B<Exhibit.RemoteListFacet._instances.length){D=Exhibit.RemoteListFacet._instances[B];
D._uiContext=A;
return D;
}var D=new Exhibit.RemoteListFacet(C,A);
Exhibit.RemoteListFacet._configure(D,E);
D._initializeUI();
A.getCollection().addFacet(D);
return D;
};
Exhibit.RemoteListFacet.createFromDOM=function(A,F,H){var E=Exhibit.getConfigurationFromDOM(A);
var H=Exhibit.UIContext.createFromDOM(A,H);
var C=Exhibit.getAttribute(A,"remoteFacetId");
if(C!=null&&C.length>0&&Exhibit.RemoteListFacet._instances.length>0&&C<Exhibit.RemoteListFacet._instances.length){var B=Exhibit.RemoteListFacet._instances[C];
B._uiContext=H;
return B;
}var B=new Exhibit.RemoteListFacet(F!==null?F:A,H);
Exhibit.SettingsUtilities.collectSettingsFromDOM(A,Exhibit.RemoteListFacet._settingSpecs,B._settings);
try{var J=Exhibit.getAttribute(A,"sourceUrl");
if(J!=null&&J.length>0){B._settings.sourceUrl=J;
}var L=Exhibit.getAttribute(A,"sourceUrlType");
if(L!=null&&L.length>0){B._settings.sourceUrlType=L;
}var N=Exhibit.getAttribute(A,"sourceExpression");
if(N!=null&&N.length>0){B._settings.sourceExpression=Exhibit.ExpressionParser.parse(N);
}var D=Exhibit.getAttribute(A,"queryParamName");
if(D!=null&&D.length>0){B._settings.queryParamName=D;
}var G=Exhibit.getAttribute(A,"queryParamSeparator");
if(G!=null&&G.length>0){B._settings.queryParamSeparator=G;
}var M=Exhibit.getAttribute(A,"selection",";");
if(M!=null&&M.length>0){for(var I=0,O;
O=M[I];
I++){B._valueSet.add(O);
}}}catch(K){SimileAjax.Debug.exception(K,"RemoteListFacet: Error processing configuration of remote list facet");
}Exhibit.RemoteListFacet._configure(B,E);
B._initializeUI();
H.getCollection().addFacet(B);
return B;
};
Exhibit.RemoteListFacet._configure=function(G,F){Exhibit.SettingsUtilities.collectSettings(F,Exhibit.RemoteListFacet._settingSpecs,G._settings);
Exhibit.RemoteListFacet._instances.push(G);
G._remoteFacetId=Exhibit.RemoteListFacet._instances.length-1;
G._div.setAttribute("ex:remoteFacetId",Exhibit.RemoteListFacet._instances.length-1);
if("sourceUrl" in F){G._settings.sourceUrl=F.sourceUrl;
}if("sourceUrlType" in F){G._settings.sourceUrlType=F.sourceUrlType;
}if("sourceExpression" in F){G._settings.sourceExpression=Exhibit.ExpressionParser.parse(F.sourceExpression);
}if("queryParamName" in F){G._settings.queryParamName=F.queryParamName;
}if("queryParamSeparator" in F){G._settings.queryParamSeparator=F.queryParamSeparator;
}if("selection" in F){var D=F.selection;
for(var C=0;
C<D.length;
C++){G._valueSet.add(D[C]);
}}if("fixedOrder" in G._settings){var A=G._settings.fixedOrder.split(";");
var E={};
for(var C=0;
C<A.length;
C++){E[A[C].trim()]=C;
}G._orderMap=E;
}if("colorCoder" in G._settings){G._colorCoder=G._uiContext.getExhibit().getComponent(G._settings.colorCoder);
}if(G._settings.collapsed){G._settings.collapsible=true;
}var B=function(){if(G._database!==null&&G._database.getAllItems().size()>0){G._dom.valuesContainer.style.display="none";
G._dom.valuesContainer.innerHTML="";
G._constructBody(G._computeFacet(G._database.getAllItems()));
G._dom.valuesContainer.style.display="block";
}};
G._database=Exhibit.Database.create();
G._database.addListener({onAfterLoadingItems:B});
G._database._loadLinks([{href:G._settings.sourceUrl,type:G._settings.sourceUrlType}],G._database,null);
};
Exhibit.RemoteListFacet.prototype.dispose=function(){this._uiContext.getCollection().removeFacet(this);
this._uiContext=null;
this._colorCoder=null;
this._div.innerHTML="";
this._div=null;
...
Exhibit.RemoteListFacet.prototype.hasRestrictions=function(){return false;
};
Exhibit.RemoteListFacet.prototype.clearAllRestrictions=function(){var A={selection:[]};
if(this._valueSet.size()>0){this._valueSet.visit(function(B){A.selection.push(B);
});
this._valueSet=new Exhibit.Set();
this.updateFacet();
}return A;
};
Exhibit.RemoteListFacet.prototype.applyRestrictions=function(B){this._valueSet=new Exhibit.Set();
for(var A=0;
A<B.selection.length;
A++){this._valueSet.add(B.selection[A]);
}this.updateFacet();
};
Exhibit.RemoteListFacet.prototype.setSelection=function(B,A){if(A){this._valueSet.add(B);
}else{this._valueSet.remove(B);
}};
Exhibit.RemoteListFacet.prototype.restrict=function(A){};
Exhibit.RemoteListFacet.prototype.update=function(){};
Exhibit.RemoteListFacet.prototype.updateFacet=function(){if(this._database!==null&&this._database.getAllItems().size()>0){this._dom.valuesContainer.style.display="none";
this._dom.valuesContainer.innerHTML="";
this._updateMainDataSource();
this._constructBody(this._computeFacet(this._database.getAllItems()));
this._dom.valuesContainer.style.display="block";
}};
Exhibit.RemoteListFacet.prototype._updateMainDataSource=function(){var H=this;
var B=function(M,N){var I="^"+H._settings.queryParamName;
var L=new RegExp(I);
var K=M.split("?");
if(K.length>1){var O=K[1];
var Q=O.split("&");
var P=[];
var J=false;
$.each(Q,function(){if(this.match(L)){P.push(H._settings.queryParamName+"="+N);
J=true;
}else{P.push(this);
}});
if(!J){P.push(H._settings.queryParamName+"="+N);
}return[K[0],"?",P.join("&")].join("");
}else{return[M,"?",H._settings.queryParamName,"=",N].join("");
}};
...
A<F.length;
A++){var E=F[A];
if(E.rel.match(/\bexhibit\/data\b/)){E.href=B(E.href,this._valueSet.toArray().join(H._settings.queryParamSeparator));
}}}var C=function(){window.exhibit=Exhibit.create();
window.exhibit.configureFromDOM();
};
window.database=Exhibit.Database.create();
window.database.loadDataLinks(C);
};
Exhibit.RemoteListFacet.prototype._computeFacet=function(C){var B=this._settings.sourceExpression.evaluate({"value":C},{value:"item"},"value",this._database);
var A=[];
if(B.size>0){var F=B.values.toArray();
var E=this._valueSet;
for(var D=0;
D<F.length;
D++){A.push({selectionLabel:F[D],actionLabel:F[D],value:F[D],selected:E.contains(F[D])});
}A.sort(this._createSortFunction(B.valueType));
}return A;
};
Exhibit.RemoteListFacet.prototype._initializeUI=function(){var A=this;
this._dom=Exhibit.FacetUtilities[this._settings.scroll?"constructFacetFrame":"constructFlowingFacetFrame"](this,this._div,this._settings.facetLabel,function(C,B,D){A._clearSelections();
},this._uiContext,this._settings.collapsible,this._settings.collapsed);
if("height" in this._settings&&this._settings.scroll){this._dom.valuesContainer.style.height=this._settings.height;
}};
Exhibit.RemoteListFacet.prototype._constructBody=function(A){var B=this;
var F=this._dom.valuesContainer;
F.style.display="none";
var D=this._valueSet.size()>0;
var E=function(J){var G=function(L,K,M){B._filter(J.value,J.actionLabel,false);
SimileAjax.DOM.cancelEvent(K);
return false;
};
var I=function(L,K,M){B._filter(J.value,J.actionLabel,!(K.ctrlKey||K.metaKey));
SimileAjax.DOM.cancelEvent(K);
return false;
};
var H=Exhibit.RemoteListFacet.constructFacetItem(J.selectionLabel,(B._colorCoder!=null)?B._colorCoder.translate(J.value):null,J.selected,B._settings.selectMultiple,D,G,I,B._uiContext);
F.appendChild(H);
};
for(var C=0;
C<A.length;
...
Exhibit.RemoteListFacet.prototype._filter=function(K,I,B){var M=this;
var C,J,H;
...
var G;
var D;
if(K!=null){G=L.contains(K);
D=G&&(L.size()==1);
if(B){N=new Exhibit.Set();
if(!L.contains(K)){N.add(K);
}else{if(L.size()>1){N.add(K);
}}}else{N=new Exhibit.Set(L);
if(N.contains(K)){N.remove(K);
}else{N.add(K);
}}}var E={selection:N.toArray()};
var F={selection:L.toArray()};
SimileAjax.History.addLengthyAction(function(){M.applyRestrictions(E);
},function(){M.applyRestrictions(F);
},(B&&!D)?String.substitute(Exhibit.FacetUtilities.l10n["facetSelectOnlyActionTitle"],[I,this._settings.facetLabel]):String.substitute(Exhibit.FacetUtilities.l10n[G?"facetUnselectActionTitle":"facetSelectActionTitle"],[I,this._settings.facetLabel]));
};
Exhibit.RemoteListFacet.prototype._clearSelections=function(){var B={};
var A=this;
SimileAjax.History.addLengthyAction(function(){B.restrictions=A.clearAllRestrictions();
},function(){A.applyRestrictions(B.restrictions);
},String.substitute(Exhibit.FacetUtilities.l10n["facetClearSelectionsActionTitle"],[this._settings.facetLabel]));
};
Exhibit.RemoteListFacet.prototype._createSortFunction=function(E){var C=function(G,F){return G.selectionLabel.localeCompare(F.selectionLabel);
};
if("_orderMap" in this){var D=this._orderMap;
C=function(G,F){if(G.selectionLabel in D){if(F.selectionLabel in D){return D[G.selectionLabel]-D[F.selectionLabel];
}else{return -1;
...
Exhibit.ServerSideFacet._settingSpecs={"facetLabel":{type:"text"},"fixedOrder":{type:"text"},"sortMode":{type:"text",defaultValue:"value"},"sortDirection":{type:"text",defaultValue:"forward"},"showMissing":{type:"boolean",defaultValue:true},"missingLabel":{type:"text"},"scroll":{type:"boolean",defaultValue:true},"height":{type:"text"},"colorCoder":{type:"text",defaultValue:null},"collapsible":{type:"boolean",defaultValue:false},"collapsed":{type:"boolean",defaultValue:false}};
Exhibit.ServerSideFacet.create=function(D,B,A){var A=Exhibit.UIContext.create(D,A);
var C=new Exhibit.ServerSideFacet(B,A);
Exhibit.ServerSideFacet._configure(C,D);
C._initializeUI();
A.getCollection().addFacet(C);
return C;
};
Exhibit.ServerSideFacet.createFromDOM=function(A,E,G){var D=Exhibit.getConfigurationFromDOM(A);
var G=Exhibit.UIContext.createFromDOM(A,G);
var B=new Exhibit.ServerSideFacet(E!=null?E:A,G);
Exhibit.SettingsUtilities.collectSettingsFromDOM(A,Exhibit.ServerSideFacet._settingSpecs,B._settings);
try{var F=Exhibit.getAttribute(A,"expression");
if(F!=null&&F.length>0){B._expression=Exhibit.ExpressionParser.parse(F);
}var C=Exhibit.getAttribute(A,"countExpression");
if(C!=null&&C.length>0){B._countExpression=Exhibit.ExpressionParser.parse(C);
}var K=Exhibit.getAttribute(A,"selection",";");
if(K!=null&&K.length>0){for(var H=0,L;
L=K[H];
H++){B._valueSet.add(L);
}}var I=Exhibit.getAttribute(A,"selectMissing");
if(I!=null&&I.length>0){B._selectMissing=(I=="true");
}}catch(J){SimileAjax.Debug.exception(J,"ServerSideFacet: Error processing configuration of list facet");
}Exhibit.ServerSideFacet._configure(B,D);
B._initializeUI();
G.getCollection().addFacet(B);
return B;
};
Exhibit.ServerSideFacet._configure=function(H,G){Exhibit.SettingsUtilities.collectSettings(G,Exhibit.ServerSideFacet._settingSpecs,H._settings);
if("expression" in G){H._expression=Exhibit.ExpressionParser.parse(G.expression);
}if("countExpression" in G){H._countExpression=Exhibit.ExpressionParser.parse(G.countExpression);
}if("selection" in G){var C=G.selection;
for(var B=0;
B<C.length;
B++){H._valueSet.add(C[B]);
...
H._getSelectionFromLinks();
};
Exhibit.ServerSideFacet.prototype.dispose=function(){this._cache.dispose();
this._cache=null;
this._uiContext.getCollection().removeFacet(this);
this._uiContext=null;
this._colorCoder=null;
...
Exhibit.ServerSideFacet.prototype.hasRestrictions=function(){return this._valueSet.size()>0||this._selectMissing;
};
Exhibit.ServerSideFacet.prototype.clearAllRestrictions=function(){var A={selection:[],selectMissing:false};
if(this.hasRestrictions()){this._valueSet.visit(function(B){A.selection.push(B);
});
this._valueSet=new Exhibit.Set();
A.selectMissing=this._selectMissing;
...
Exhibit.ServerSideFacet.prototype.applyRestrictions=function(B){this._valueSet=new Exhibit.Set();
for(var A=0;
A<B.selection.length;
A++){this._valueSet.add(B.selection[A]);
}this._selectMissing=B.selectMissing;
...
Exhibit.ServerSideFacet.prototype.setSelection=function(B,A){if(A){this._valueSet.add(B);
}else{this._valueSet.remove(B);
}this._notifyCollection();
};
Exhibit.ServerSideFacet.prototype.setSelectMissing=function(A){if(A!=this._selectMissing){this._selectMissing=A;
this._notifyCollection();
}};
Exhibit.ServerSideFacet.prototype.restrict=function(A){if(this._valueSet.size()==0&&!this._selectMissing){return A;
}var B=this._cache.getItemsFromValues(this._valueSet,A);
if(this._selectMissing){this._cache.getItemsMissingValue(A,B);
}return B;
};
Exhibit.ServerSideFacet.prototype.update=function(A){this._dom.valuesContainer.style.display="none";
this._dom.valuesContainer.innerHTML="";
this._constructBody(this._computeFacet(A));
this._dom.valuesContainer.style.display="block";
};
Exhibit.ServerSideFacet.prototype._computeFacet=function(F){var K=this._uiContext.getDatabase();
var A=this._cache.getValueCountsFromItems(F);
var D=A.entries;
var B=A.valueType;
if(D.length>0){var J=this._valueSet;
...
Exhibit.ServerSideFacet.prototype._notifyCollection=function(){var B=function(M,J,N){var I="^"+J;
var L=new RegExp(I);
var K=M.split("?");
if(K.length>1){var P=K[1];
var Q=P.split("&");
var O=[];
$.each(Q,function(){if(this.match(L)){O.push(J+"="+N);
}else{O.push(this);
}});
return[K[0],"?",O.join("&")].join("");
}else{return[M,"?",J,"=",N].join("");
}};
var H=[];
this._valueSet.visit(function(I){H.push(I);
});
var G=document.documentElement.getElementsByTagName("head");
for(var D=0;
D<G.length;
...
A<F.length;
A++){var E=F[A];
if(E.rel.match(/\bexhibit\/data\b/)){E.href=B(E.href,this._cache._collection._itemTypes,H.join(","));
}}}var C=function(){window.exhibit.configureFromDOM();
};
window.database=Exhibit.Database.create();
window.database.loadDataLinks(C);
};
Exhibit.ServerSideFacet.prototype._initializeUI=function(){var A=this;
...
Exhibit.ServerSideFacet.prototype._constructBody=function(A){var B=this;
var G=this._dom.valuesContainer;
G.style.display="none";
var F=Exhibit.ServerSideFacet[this._settings.scroll?"constructFacetItem":"constructFlowingFacetItem"];
var D=this._valueSet.size()>0||this._selectMissing;
var E=function(K){var H=function(M,L,N){B._filter(K.value,K.actionLabel,false);
SimileAjax.DOM.cancelEvent(L);
return false;
...
var I=F(K.selectionLabel,(B._colorCoder!=null)?B._colorCoder.translate(K.value):null,K.selected,D,H,J,B._uiContext);
G.appendChild(I);
};
for(var C=0;
C<A.length;
...
Exhibit.ServerSideFacet.prototype._filter=function(M,J,B){var O=this;
var C,K,H;
var N=new Exhibit.Set(this._valueSet);
var L=this._selectMissing;
var P;
...
Exhibit.ServerSideFacet.prototype._clearSelections=function(){var B={};
var A=this;
SimileAjax.History.addLengthyAction(function(){B.restrictions=A.clearAllRestrictions();
},function(){A.applyRestrictions(B.restrictions);
},String.substitute(Exhibit.FacetUtilities.l10n["facetClearSelectionsActionTitle"],[this._settings.facetLabel]));
};
Exhibit.ServerSideFacet.prototype._createSortFunction=function(E){var C=function(G,F){return G.selectionLabel.localeCompare(F.selectionLabel);
};
if("_orderMap" in this){var D=this._orderMap;
C=function(G,F){if(G.selectionLabel in D){if(F.selectionLabel in D){return D[G.selectionLabel]-D[F.selectionLabel];
}else{return -1;
...
Exhibit.ServerSideFacet.prototype._getSelectionFromLinks=function(){var I=document.documentElement.getElementsByTagName("head");
for(var D=0;
D<I.length;
D++){var F=I[D].getElementsByTagName("link");
for(var C=0;
C<F.length;
C++){var H=F[C];
if(H.rel.match(/\bexhibit\/data\b/)){var E=H.href.split("?");
if(E.length>1){var A="^"+this._cache._collection._itemTypes;
var G=new RegExp(A);
var J=E[1].split("&");
var B=this;
$.each(J,function(){if(this.match(G)){var K=this.split("=")[1].split(",");
$.each(K,function(){var L=decodeURI(this);
if(!B._valueSet.contains(L)){B._valueSet.add(L);
}});
}});
}}}}};
Exhibit.ServerSideFacet.constructFacetItem=function(E,B,G,D,C,F,A){if(Exhibit.params.safe){E=Exhibit.Formatter.encodeAngleBrackets(E);
}var H=SimileAjax.DOM.createDOMFromString("div","<div class='exhibit-facet-value-inner' id='inner'>"+("<div class='exhibit-facet-value-checkbox'>&#160;"+SimileAjax.Graphics.createTranslucentImageHTML(Exhibit.urlPrefix+(D?(G?"images/black-check.png":"images/no-check.png"):"images/no-check-no-border.png"))+"</div>")+"<a class='exhibit-facet-value-link' href='javascript:{}' id='link'></a></div>");
H.elmt.className=G?"exhibit-facet-value exhibit-facet-value-selected":"exhibit-facet-value";
if(typeof E=="string"){H.elmt.title=E;
H.link.appendChild(document.createTextNode(E));
if(B!=null){H.link.style.color=B;
}}else{H.link.appendChild(E);
if(B!=null){E.style.color=B;
}}SimileAjax.WindowManager.registerEvent(H.elmt,"click",F,SimileAjax.WindowManager.getBaseLayer());
if(D){SimileAjax.WindowManager.registerEvent(H.inner.firstChild,"click",C,SimileAjax.WindowManager.getBaseLayer());
}return H.elmt;
};
Exhibit.ServerSideFacet.constructFlowingFacetItem=function(E,B,G,D,C,F,A){if(Exhibit.params.safe){E=Exhibit.Formatter.encodeAngleBrackets(E);
}var H=SimileAjax.DOM.createDOMFromString("div",("<div class='exhibit-flowingFacet-value-checkbox'>"+SimileAjax.Graphics.createTranslucentImageHTML(Exhibit.urlPrefix+(D?(G?"images/black-check.png":"images/no-check.png"):"images/no-check-no-border.png"))+"</div>")+"<a class='exhibit-flowingFacet-value-link' href='javascript:{}' id='inner'></a>");
H.elmt.className=G?"exhibit-flowingFacet-value exhibit-flowingFacet-value-selected":"exhibit-flowingFacet-value";
if(typeof E=="string"){H.elmt.title=E;
H.inner.appendChild(document.createTextNode(E));
if(B!=null){H.inner.style.color=B;
}}else{H.inner.appendChild(E);
if(B!=null){E.style.color=B;
}}SimileAjax.WindowManager.registerEvent(H.elmt,"click",F,SimileAjax.WindowManager.getBaseLayer());
if(D){SimileAjax.WindowManager.registerEvent(H.elmt.firstChild,"click",C,SimileAjax.WindowManager.getBaseLayer());
}return H.elmt;
};
...
Exhibit.CalendarView.prototype._reconstruct=function(){this._div.innerHTML="";
var N=this;
var H=this._uiContext.getCollection();
var K=this._uiContext.getDatabase();
var C=this._settings;
var F=this._accessors;
var L=H.countRestrictedItems();
var M={};
if(L>0){var J=H.getRestrictedItems();
var D=(this._accessors.getColorKey!=null);
var G=(this._accessors.getIconKey!=null&&this._iconCoder!=null);
var I=(this._accessors.getHoverText!=null);
...
var E=function(U,T,P,R,S){var Q;
F.getEventLabel(U,K,function(V){Q=V;
return true;
});
var O={itemID:U,start:T.start,end:T.end,label:Q,icon:R,color:P,hoverText:S,getProperty:function(V){return K.getObject(this._itemID,V);
},fillInfoBubble:function(V,W,X){N._fillInfoBubble(U,V,W,X);
}};
M[U]=O;
};
J.visit(function(O){var P=[];
N._getDuration(O,K,function(X){if("start" in X){P.push(X);
}});
if(P.length>0){var R=null;
var V=null;
var Q=null;
if(D){var W=new Exhibit.Set();
F.getColorKey(O,K,function(X){W.add(X);
});
R=N._colorCoder.translateSet(W,B);
}var V=null;
if(G){var U=new Exhibit.Set();
F.getIconKey(O,K,function(X){U.add(X);
});
V=N._iconCoder.translateSet(U,A);
}if(I){var S=new Exhibit.Set();
F.getHoverText(O,K,function(X){S.add(X);
});
for(var T in S._hash){Q=T;
}}for(var T=0;
T<P.length;
T++){E(O,P[T],R,V,Q);
}}});
}this._div.style.display="none";
this._div.appendChild(this.buildCal(M));
...
Exhibit.CalendarView.prototype.buildCal=function(R){var Q=this;
...
Exhibit.CalendarView.prototype.buildCell=function(B,M,G,A){try{var L=this;
...
J.className="event-title";
cellLink=document.createElement("a");
cellLink.href="javascript:{}";
cellLink.setAttribute("ex:itemID",K.itemID);
cellLink.innerHTML=K.label;
SimileAjax.WindowManager.registerEvent(cellLink,"click",function(O,N,P){L._fillInfoBubble(O,[O.getAttribute("ex:itemID")]);
SimileAjax.DOM.cancelEvent(N);
return false;
},SimileAjax.WindowManager.getBaseLayer());
J.appendChild(cellLink);
D.appendChild(J);
...
SimileAjax.WindowManager.registerEvent(E,"click",function(O,N,P){L._fillInfoBubble(O,A);
SimileAjax.DOM.cancelEvent(N);
return false;
},SimileAjax.WindowManager.getBaseLayer());
C.appendChild(E);
D.appendChild(C);
}}D.className=[M,"day",((B.getDay()===0||B.getDay()==6)?"weekend":"")].join(" ");
return D;
}catch(F){alert("buildCell: "+F.message);
}};
Exhibit.CalendarView.prototype._fillInfoBubble=function(E,G){var K=SimileAjax.DOM.getPageCoordinates(E);
var C=document.createElement("div");
if(G.length==1){this._uiContext.getLensRegistry().createLens(G[0],C,this._uiContext);
}else{var O=this;
var I=this._accessors;
var N=this._uiContext.getDatabase();
var F=[];
this._getDuration(G[0],N,function(P){if("start" in P){F.push(P);
}});
var H=document.createElement("div");
H.innerHTML=Exhibit.DateUtil.formatDate(F[0].start,"E, MMM d, yyyy");
...
L<G.length;
L++){D=G[L];
I.getEventLabel(D,N,function(P){M=P;
return true;
});
...
B.innerHTML=M;
B.href="javascript:{}";
B.setAttribute("ex:itemID",D);
SimileAjax.WindowManager.registerEvent(B,"click",function(Q,P,R){O._fillInfoBubble(E,[Q.getAttribute("ex:itemID")]);
SimileAjax.DOM.cancelEvent(P);
return false;
},SimileAjax.WindowManager.getBaseLayer());
A.appendChild(B);
...
SimileAjax.Graphics.createBubbleForContentAndPoint(C,K.left+Math.ceil(E.offsetWidth/2),K.top+Math.ceil(E.offsetHeight/2),400);
};
...
var B=C?1:-1;
var J=function(Q,P){if(Q.sortKey===Number.NEGATIVE_INFINITY&&P.sortKey===Number.NEGATIVE_INFINITY){return Number.NEGATIVE_INFINITY;
}else{return B*(Q.sortKey-P.sortKey);
}};
...
}else{if(sType=="string"){var n=parseInt(s,10);
if(!isNaN(n)){return n;
...
Exhibit.SettingsUtilities._intParser=function(A,B){var C=parseInt(A,10);
if(!isNaN(C)){return B(C);
}return false;
};
