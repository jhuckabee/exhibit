...
for(var C in this._componentMap){if(C.match(/facet/)&&(typeof this._componentMap[C].exportSettings!=="undefined")){B=this._componentMap[C].exportSettings()||false;
if(B){A.push('"'+C+'": '+B);
}}}return"{"+A.join(", ")+"}";
};
Exhibit._Impl.prototype.importSettings=function(settings){var facetSettings=eval("("+settings+")");
for(var id in facetSettings){if(this._componentMap[id]&&(typeof this._componentMap[id].importSettings!=="undefined")){this._componentMap[id].importSettings(facetSettings[id]);
}}};
...
try{var E=Exhibit.getAttribute(A,"expression");
if(E!=null&&E.length>0){B._expression=Exhibit.ExpressionParser.parse(E);
}var F=Exhibit.getAttribute(A,"timerLimit");
if(F!=null&&F.length>0){B._datePickerTimerLimit=F;
}var I=Exhibit.getAttribute(A,"dragSelection");
if(I!=null&&I.length>0){B._enableDragSelection=(I=="true");
...
if(B._facet._enableDragSelection){SimileAjax.WindowManager.registerEvent(D.elmt,"mousedown",function(F,E,G){B._facet.selectDate(Exhibit.DatePickerFacet.DateUtil.formatDate(C,B._facet._dateFormat));
SimileAjax.DOM.cancelEvent(E);
return false;
},SimileAjax.WindowManager.getBaseLayer());
SimileAjax.WindowManager.registerEvent(D.elmt,"mouseup",function(F,E,G){B._facet.selectDate(Exhibit.DatePickerFacet.DateUtil.formatDate(C,B._facet._dateFormat));
SimileAjax.DOM.cancelEvent(E);
return false;
},SimileAjax.WindowManager.getBaseLayer());
}else{SimileAjax.WindowManager.registerEvent(D.elmt,"click",function(F,E,G){B._facet.selectDate(Exhibit.DatePickerFacet.DateUtil.formatDate(C,B._facet._dateFormat));
SimileAjax.DOM.cancelEvent(E);
return false;
},SimileAjax.WindowManager.getBaseLayer());
...
Exhibit.ListFacet.prototype.exportSettings=function(){var A=[];
this._valueSet.visit(function(B){A.push(B);
});
if(A.length>0){return'["'+A.join('","')+'"]';
}};
Exhibit.ListFacet.prototype.importSettings=function(B){var A=this;
A.applyRestrictions({selection:B,selectMissing:A._selectMissing});
};
